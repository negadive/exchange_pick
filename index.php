...
if(isset($_POST['nick'])){
	...
	$querycekinven = mysqli_query($con, $sqlcekinven)or mysqli_error();
	if(!mysqli_num_rows($querycekinven)=="0"){
		...
		if((isset($_POST['hehe3'])) && (!empty($_POST['item']))){
			...
			if(($row["it".$no[0]] == $i1 && $row["it".$no[1]] == $i2) || (("it".$row[$no[0]] == $i2 && $row["it".$no[1]] == $i1))){
				...
				$quelect = mysqli_query($con, $sqlect)or mysqli_error($sqlect);

				if($quelect){
					$sqlect = " update store set it".$no3."='$hs' where user_id='$nick'";
					$quelect2 = mysqli_query($con, $sqlect)or mysqli_error($sqlect);
